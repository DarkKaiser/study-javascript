// Copyright 1995-2014 Daum Communications Corp.

var __Tiara=__Tiara||{};__Tiara.version='2.7.3.5';__Tiara.enumerables=['hasOwnProperty','valueOf','isPrototypeOf','propertyIsEnumerable','toLocaleString','toString','constructor'];__Tiara.apply=function(o,c,defaults){'use strict';var p;if(defaults){__Tiara.apply(o,defaults);}
...
return params;},getParams=function(){var param=new tiara.util.StringBuffer();for(var key in getParamsObject()){if(__params.hasOwnProperty(key)&&__params[key]!==''){param.append(encode(key)+'='+encode(__params[key]));}}
if(param.isEmpty()){return'';}
return'&'+param.join('&');},addEvent=function(target,eventName,handler,capture){if(!target){return;}
if(target.addEventListener){target.addEventListener(eventName,handler,!!capture);}else if(target.attachEvent){target.attachEvent('on'+eventName,handler);}else{var oldevent=target['on'+eventName];if('function'===typeof oldevent){target['on'+eventName]=function(){oldevent();handler();};}else{target['on'+eventName]=handler;}}},pageViewSender=function(url){var img=new Image(1,1);img.onload=function(){img.onload=null;img.onerror=null;};img.onerror=function(){img.onload=null;img.onerror=null;var img2=new Image(1,1);img2.src=url;img2.onload=null;img2.onerror=null;};img.src=url;},appendTiaraFrame=function(dom,frame_id){var tiara_frame=__d.getElementById(frame_id);if(!tiara_frame){tiara_frame=__d.createElement('iframe');tiara_frame.id=frame_id;tiara_frame.height='0';tiara_frame.width='0';tiara_frame.style.display='none';tiara_frame.style.visibility='hidden';dom.appendChild(tiara_frame);}
return tiara_frame;},trackPageview=function(nURL){if(__sendPV||__isBot){return;}
...
if(__w.location.protocol==='https:'||__isChrome||__isIOS||__w.location.hostname==='search.daum.net'||__w.location.hostname.indexOf('kakao.com')>-1){pageViewSender(callURL.toString());return;}
var minidaumLayer=__d.getElementById('daumMinidaum')||__d.getElementById('wrapMinidaum')||__d.getElementById('DaumUI__minidaum');var tiaraLayer=__d.getElementById(__tiara_layer_id);var dom=tiaraLayer||minidaumLayer;if(!dom){pageViewSender(callURL.toString());return;}
var tiara_frame=__d.getElementById(__tiara_frame_id);if(tiara_frame===null){tiara_frame=appendTiaraFrame(dom,__tiara_frame_id);}
if(!tiara_frame){pageViewSender(callURL.toString());}else{dummy='';if('string'===typeof __w.location.search&&__w.location.search.indexOf('debug')>-1){dummy+='&debug';}
tiara_frame.src=[__w.location.protocol,'//track.tiara.daum.net/shining/index.shtml?dummy=',dummy].join('');addEvent(tiara_frame,'load',function(url,tiara_frame,__tiara_frame_id){return function(){var img=new Image(1,1);img.onload=function(){};img.onerror=function(){var img2=new Image(1,1);img2.src=url;};img.src=url;var frame=tiara_frame||document.getElementById(__tiara_frame_id);if(frame&&frame.parentNode){frame.parentNode.removeChild(frame);}
frame=null;img=null;};}(callURL.toString(),tiara_frame,__tiara_frame_id),false);}
tiara_frame=null;},getCookie=function(name){var ckName=name+'=';var cookie=__d.cookie;if(cookie){var start=cookie.indexOf(ckName);var end;if(start!==-1){start+=ckName.length;end=cookie.indexOf(';',start);if(end===-1){end=cookie.length;}
...
__referer=referer;__setRefererForce=true;},__setConfig:function(){if(arguments.length===1&&typeof arguments[0]==='object'){__Tiara.apply(__conf,arguments[0]);}},__setAllowHash:function(bool){__conf.enableHash=!!bool;__params._spa=1;},__addParam:function(key,value){if(key&&value){if(checkMetaName(key)&&'string'===typeof value){if(__metaitems){__metaitems[cutString(key,50)]=cutString(value,127);}}else{__params[key]=value;}}},__getMetaItems:function(){return __metaitems;},__pageView:function(obj){var metaItems=__Tiara.clone(__metaitems),paramObject=__Tiara.apply(getParamsObject(),obj);return pageViewSender([__h,toKeyValue(__Tiara.apply(metaItems,paramObject))].join(''));},__trackPageview:function(nURL){trackPageview.apply(null,arguments);},__trackPageshow:function(){__sendPV=false;trackPageview.apply(null,arguments);},onHashChangeEvent:function(evt){if(evt.newURL&&evt.oldURL){var newURL=evt.newURL,oldURL=evt.oldURL;if(!!__conf.disableQuery){var pos=newURL.indexOf('?');newURL=pos===-1?newURL:newURL.substring(0,pos);pos=oldURL.indexOf('?');oldURL=pos===-1?oldURL:oldURL.substring(0,pos);}
...
v=partial.length===0?'{}':'{'+partial.join(',')+'}';return v;}}
if(typeof __Tiara.util.JSON.stringify!=='function'){if(window.JSON&&'function'===typeof window.JSON.stringify){__Tiara.util.JSON.stringify=window.JSON.stringify;}else{__Tiara.util.JSON.stringify=function(value){return str('',{'':value});};}}}());var _tiq=_tiq||[];__Tiara.conf=__Tiara.conf||{};__Tiara.conf.q=_tiq||[];__Tiara.ring=function(conf){'use strict';var DOMPATH_REG=/#([0-9a-zA-Z_\.\-]+)/g,DOMID_REG=/^([0-9a-zA-Z_\.\-]+)$/g,CONTENTNO_REG=/@([0-9a-zA-Z_:\.\-]+)/,__conf=conf||{},__queue=__conf.q||[],puid=__conf.puid||new Date().getTime(),__params={puid:puid,sections:'Default',pname:'Default',version:__Tiara.version},__ua=navigator.userAgent.toLowerCase(),__isMobile=/ip[ao]d|iphone|android/i.test(__ua),__tiara_layer_id='__tiara_layer',__tiara_click_frame_id='__tiara_click_frame',__w=window,__d=document,__h=[__w.location.hostname.indexOf('.kakao.com')>0?'https://click.tiara.kakao.com':'https://click.tiara.daum.net','/queen/touch?'].join(''),pageTracker=__Tiara.__getTracker(),regFind=function(reg,str){return reg.exec(str);},getAbsolutePosition=function(element){var r={x:element.offsetLeft,y:element.offsetTop};if(element.offsetParent){var tmp=getAbsolutePosition(element.offsetParent);r.x+=tmp.x;r.y+=tmp.y;}
...
return tiara_frame;},eventSender=function(urlParams){var img=new Image(1,1);img.onload=function(){img.onload=null;img.onerror=null;};img.onerror=function(){img.onload=null;img.onerror=null;var img2=new Image(1,1);img2.src=[__h,urlParams,getParams(),getDocumentCharset()].join('');img2.onload=null;img2.onerror=null;};img.src=[__h,urlParams,getParams(),getDocumentCharset()].join('');},touchSender=function(objParams){var params=__Tiara.clone(__params);var args=__Tiara.apply(__Tiara.apply(params,pageTracker.__getMetaItems()),objParams);var img=new Image(1,1);img.onload=function(){img.onload=null;img.onerror=null;};img.onerror=function(){img.onload=null;img.onerror=null;var img2=new Image(1,1);img2.src=[__h,toKeyValue(getParamsInfo(args)),getDocumentCharset()].join('');img2.onload=null;img2.onerror=null;};img.src=[__h,toKeyValue(getParamsInfo(args)),getDocumentCharset()].join('');},clickSender=function(urlParams){if(__isMobile){eventSender(urlParams);return true;}
var tiara_frame=__d.getElementById(__tiara_click_frame_id);if(!tiara_frame){var miniDaumLayer=__d.getElementById('daumMinidaum')||__d.getElementById('wrapMinidaum')||__d.getElementById('DaumUI__minidaum');var tiaraLayer=__d.getElementById(__tiara_layer_id);var dom=tiaraLayer||miniDaumLayer;if(dom){tiara_frame=appendTiaraFrame(dom,__tiara_click_frame_id);}}
if(tiara_frame&&'undefined'!==tiara_frame.tagName&&tiara_frame.tagName==='IFRAME'){tiara_frame.src=[__h,urlParams,getParams(),getDocumentCharset(),"&jstime=if"].join('');addEvent(tiara_frame,'load',function(tiara_frame,__tiara_click_frame_id){return function(){var frame=tiara_frame||document.getElementById(__tiara_click_frame_id);if(frame&&frame.parentNode){var new_frame=document.createElement('iframe');new_frame.id=__tiara_click_frame_id;new_frame.height='0';new_frame.width='0';new_frame.style.display='none';new_frame.style.visibility='hidden';frame.parentNode.replaceChild(new_frame,frame);new_frame=null;}
frame=null;};}(tiara_frame,__tiara_click_frame_id),false);}else{eventSender(urlParams);}
return true;},trackClick=function(){var params=[];switch(arguments.length){case 1:params.push('dpath='+encodeURIComponent(arguments[0]));params.push('curl='+encodeURIComponent(location.href));break;case 2:params.push('dpath='+encodeURIComponent(arguments[0]));params.push('curl='+encodeURIComponent(arguments[1]));break;default:break;}
...
if(window.event&&'object'===typeof window.event){var evt=window.event,node=evt.srcElement||evt.target;screenAndClickPosition(evt,info);domTreeAndPath(node,info);}
if(params.length>0){params.push('curl='+encodeURIComponent(location.href));return eventSender([params.join('&'),'&',toKeyValue(info)].join(''));}
return true;},getParamsInfo=function(ex_params){var param={},hasSvcDomain=false;for(var key in __params){if(__params.hasOwnProperty(key)&&__params[key]){param[key]=__params[key];if('svcdomain'===key){hasSvcDomain=true;}}}
var items=pageTracker.__getMetaItems();if(!hasSvcDomain&&items&&'string'===typeof items.svcdomain){param.svcdomain=items.svcdomain;}
if(!!__conf.enableHash){param.url=__w.location.href;}
if(!!__conf.disableQuery){var pos=param.url.indexOf('?');param.url=pos===-1?param.url:param.url.substring(0,pos);}
